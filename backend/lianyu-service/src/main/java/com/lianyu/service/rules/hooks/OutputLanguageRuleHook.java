    ...
    public String render(PromptRuleContext context) {
        return switch (OutputLanguage.fromCode(context.outputLanguage())) {
            ...
            case ZH_TW -> """
                    ...
                    【硬性禁令】除極少數專有名詞、角色外號原文外，禁止在正文裡寫英文單詞、日文假名、韓文等外語句子。
                    即使用戶夾帶英語、或對話歷史裡出現過英語，你也必須用中文重新表述，不得輸出英文段落或整句外語。
                    【名字與人設一致性】
            ...
            case ZH -> """
                    ...
                    【硬性禁令】除极少数专有名词、角色外号原文外，禁止在正文里写英文单词、日文假名、韩文等外语句子。
                    即使用户夹带英语、或对话历史里出现过英语，你也必须用中文重新表述，不得输出英文段落或整句外语。
                    自检：若草稿里出现整句/整段外语，立刻改写成中文后再发送；违反语言规则视为严重错误。
                    【名字与人设一致性】
