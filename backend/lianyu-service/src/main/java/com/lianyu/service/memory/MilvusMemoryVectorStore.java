...
public class MilvusMemoryVectorStore implements MemoryVectorStore {
    ...
    public List<VectorHit> search(Long characterId,
        ...
        if (searchResult.getData() == null) {
            throw new IllegalStateException("Vector search returned null");
        }
        ...
        Map<Integer, Long> memoryIdByIndex = extractLongField(resultData, "memory_id");
        ...
        for (int i = 0; i < resultData.getIds().getIntId().getDataCount(); i++) {
            ...
            if (summary != null && !summary.isBlank()) {
                hits.add(new VectorHit(memoryIdByIndex.get(i), summary, score));
            }
        }
        return hits;
    }
    ...
    private Map<Integer, Long> extractLongField(SearchResultData resultData, String fieldName) {
        Map<Integer, Long> map = new HashMap<>();
        for (FieldData field : resultData.getFieldsDataList()) {
            if (!fieldName.equals(field.getFieldName())) {
                continue;
            }
            if (field.getScalars().hasLongData()) {
                List<Long> values = field.getScalars().getLongData().getDataList();
                for (int i = 0; i < values.size(); i++) {
                    map.put(i, values.get(i));
                }
            }
